...
def process_responses(input_file):
    ...
    for p in population:
        os.makedirs(f"./data/load/{population[p]}", exist_ok=True)
        ...
        for idx, daily_trace in enumerate(kwh):
            with open(f'./data/load/{population[p]}/{p}_{idx}.txt', 'a') as file:
...
def analyze_and_plot_hourly(load_files):
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig('./data/load/average_per_hour.png')
    plt.show()
...
def analyze_and_plot_monthly(load_files):
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig('./data/load/average_per_month.png')
    plt.show()
...
def get_load_files():
    ...
    for type in ["Detached", "Semi-detached", "Terraced"]:
        files = files + [f"./data/load/{type}/{f}" for f in os.listdir(f"./data/load/{type}") if f.endswith('.txt')]
    return files

if __name__ == "__main__":
    # Create output directory
    # shutil.rmtree("./data/load/")
    # os.makedirs('./data/load/', exist_ok=True)

    # # First process the raw data
    # for load_file_idx in range(365):
    #     process_responses(f'./data/faraday_raw/day_{load_file_idx}.json')
