...
def analyze_and_plot_daily(solar_files):
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig('./data/solar/average_daily_pv_production.png')
    plt.show()
...
def analyze_and_plot_monthly(solar_files):
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig('./data/solar/average_monthly_pv_production.png')
    plt.show()

def get_solar_files():
    # Changed path to go up one level
    return [f"./data/solar/{f}" for f in os.listdir("./data/solar") if f.endswith('.txt')]
