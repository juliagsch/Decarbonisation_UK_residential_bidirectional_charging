...
merged_data['Payback Time (years)'] =  merged_data['CAPEX'] / merged_data['OPEX Savings']
...
def create_payback_time_chart():
    ...
    for cah in cah_types:  # H1 and H2
        for archetype in archetype_order:
            ...
            if len(filtered_uni) >= 2:
                best_val_uni = filtered_uni[filtered_uni['Solar'] == 'best']['Payback Time (years)'].mean()
                worst_val_uni = filtered_uni[filtered_uni['Solar'] == 'worst']['Payback Time (years)'].mean()
                ci_uni = worst_val_uni - best_val_uni
                # Draw rectangle for Uni
                plt.bar(current_position, height=ci_uni, bottom=best_val_uni, width=0.4, color=color_map['Unidirectional'], alpha=0.7)
            ...
            if len(filtered_bi) >= 2:
                best_val_bi = filtered_bi[filtered_bi['Solar'] == 'best']['Payback Time (years)'].mean()
                worst_val_bi = filtered_bi[filtered_bi['Solar'] == 'worst']['Payback Time (years)'].mean()
                ci_bi = worst_val_bi - best_val_bi

                plt.bar(current_position, height=ci_bi, bottom=best_val_bi, width=0.4, color=color_map['Bidirectional'], alpha=0.7)
            current_position += 1
    ...
    plt.text(1.0, 26, 'CAH1', ha='center', va='top', fontsize=14, color='black')
    plt.text(4.0, 26, 'CAH2', ha='center', va='top', fontsize=14, color='black')
    plt.text(7.0, 26, 'CAH3', ha='center', va='top', fontsize=14, color='black')
    ...
    plt.tight_layout()
    plt.savefig("./graphs/out/payback_LBN.png")
    plt.show()
